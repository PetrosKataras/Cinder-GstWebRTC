...
/*
* Minor additions from Petros Kataras, 2020.
* Added 2 new signalling server message handles for SESSION_CREATED and QUIT.
*/

/**
* @property {function} onpeerquit - Callback fired when the remote peer has disconnected.
*
*/
class WebRTCDemoSignalling {
    ...
    _onServerMessage(event) {
        ...
        if (event.data === "HELLO") {
            ...
            this._setStatus("Press to start video stream.");
            return;
        }
        ...
        if( event.data.startsWith( "SESSION_CREATED" ) ) {
            this._setStatus( "Session created: " + event.data );
            this._setStatus( "Video stream about to start." );
            return;
        }

		if( event.data.startsWith( "QUIT" ) ) {
			this._setStatus("Remote peer has quitted." );	
			this.state = 'failed';
			if( this.onpeerquit )
				this.onpeerquit();
			return;
